...
/**
 * Pequeño diálogo reutilizable para elegir una fecha (yyyy-MM-dd) usando sólo Swing estándar.
 */
public final class DatePickerUtil {
    ...
    public static LocalDate pickDate(java.awt.Component parent, LocalDate initial) {
        ...
        JDialog dialog;
        if (w instanceof Frame f) dialog = new JDialog(f, "Seleccionar fecha", true);
        else if (w instanceof Dialog d) dialog = new JDialog(d, "Seleccionar fecha", true);
        else dialog = new JDialog((Frame) null, "Seleccionar fecha", true);
    ...
    refillHolder[0] = () -> {
            ...
            for (int i=0;i<shift;i++) daysGrid.add(new JLabel(""));
        ...
        prevYear.addActionListener(e -> { currentYm[0] = currentYm[0].minusYears(1); refillHolder[0].run(); });
        nextYear.addActionListener(e -> { currentYm[0] = currentYm[0].plusYears(1); refillHolder[0].run(); });
        prevMonth.addActionListener(e -> { currentYm[0] = currentYm[0].minusMonths(1); refillHolder[0].run(); });
        nextMonth.addActionListener(e -> { currentYm[0] = currentYm[0].plusMonths(1); refillHolder[0].run(); });
        ...
        monthLabel.addMouseListener(new java.awt.event.MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent e) {
                ...
                southSel.add(okSel); southSel.add(cancelSel);
        ...
        leftBtns.add(prevYear); leftBtns.add(prevMonth);
        JPanel rightBtns = new JPanel(new java.awt.FlowLayout(java.awt.FlowLayout.RIGHT,2,2));
        rightBtns.add(nextMonth); rightBtns.add(nextYear);
    ...
    refillHolder[0].run();
        ...
        cancel.addActionListener(e -> { selected[0] = null; dialog.dispose(); });
